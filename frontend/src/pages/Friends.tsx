...
// export default function Friends() {
    ...
    let [chanName, setChanName] = useState<string>("");
    let [chanMdp, setChanMdp] = useState<string>("");

    let [joinChanName, setJoinChanName] = useState<string>("");
    let [joinChanMdp, setJoinChanMdp] = useState<string>("");
    ...
    const handleChange = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        {
            var elements = document.getElementById('create-channel-form-label');
            if (elements && elements.style.border === "1px solid red")
                elements.style.border = "1px solid green";
            else if (elements && event.target.value === "" && elements.style.border === "1px solid green")
        ...
        {
            var elements = document.getElementById('join-channel-form-label');
            if (elements && elements.style.border === "1px solid red")
                elements.style.border = "1px solid green";
            else if (elements && event.target.value === "" && elements.style.border === "1px solid green")
                elements.style.border = "1px solid red";
    ...
    const createChan = (event: any) : void => {
        ...
        if (event.target.name === 'button-create-chan')
        {
            ...
            const newChan = {name:  chanName, mdp: chanMdp};
        ...
        if (event.target.name === 'button-join-chan')
        {
            ...
            while (chansList[x])
            {
                if (chansList[x].name === joinChanName)
                    break ;
                x++;
            }
            ...
            {
                var elements = document.getElementById('join-channel-mdp-input');
                if (elements)
                    elements.style.border = "1px solid red";
                return ;
            }
            const newChan = {name:  joinChanName, mdp: joinChanMdp};
            userChans.push(newChan);
            setJoinChanName("");
            setJoinChanMdp("");
            CloseJoinChanModal();
    ...
    const activeConv = (event: any) => {
        if (event.target.classList != "wrapper-active-conv")
            return ;
        let active_elem = document.getElementsByClassName('active-conv-bg')[0];
        if (active_elem)
            active_elem.classList.toggle("active-conv-bg");
        ...
        <div className="friends-wrapper">
                ...
                <div className="chan-list-buttons">
                    ...
                    >
                        <Box className="create-chan-modal">
                            <form className='create-channel-form'>
                                <label>
                                    <input type="text" name="create-chan-name" placeholder='Name' id='create-channel-form-label' onChange={handleChange}/>
                                </label>
                                <label>
                                    <input type="text" name="create-chan-mdp" placeholder='Mot de passe' className='mdp-channel-form-label' onChange={handleChange}/>
                                </label>
                                {/* <input type="submit" value="Add" className='add-channel-form-submit-button'/> */}
                                <button name='button-create-chan' type='submit' className="redirect-button" onClick={createChan}>Create</button>
                            </form>
                        </Box>
                    </Modal>
                    <Modal
                    ...
                    >
                        <Box className="join-chan-modal">
                            <form className='join-channel-form'>
                                <label>
                                    <input type="text" name="join-chan-name" placeholder='Name' id='join-channel-form-label' onChange={handleChange}/>
                                </label>
                                <label>
                                    <input type="text" name="join-chan-mdp" placeholder='Mot de passe' id='join-channel-mdp-input' onChange={handleChange}/>
                                </label>
                                <button name='button-join-chan' type='submit' className="redirect-button" onClick={createChan}>Create</button>
                            </form>
                        </Box>
                    </Modal>
                </div>
            ...
            <div className="priv-conv-list">
                ...
                <form className='add-friend-form'>
                    ...
                    <button type="submit" value="Add" className='add-friend-form-submit-button' onClick={addFriend}>Add</button>
                </form>
            </div>
            <div className="chat">
                <Chat />
